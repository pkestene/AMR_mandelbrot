# Two alternatives:
# 1. If AMR_MANDELBROT_KOKKOS_BUILD is ON, we download kokkos sources and build them using FetchContent (which actually uses add_subdirectory)
# 2. If AMR_MANDELBROT_KOKKOS_BUILD is OFF (default), we don't build kokkos, but use find_package for setup (you must have kokkos already installed)
...
#
option(AMR_MANDELBROT_KOKKOS_BUILD "Turn ON if you want to build kokkos (default: OFF)" OFF)

#
# Option to use git (instead of tarball release) for downloading kokkos
#
option(AMR_MANDELBROT_KOKKOS_USE_GIT "Turn ON if you want to use git to download Kokkos sources (default: OFF)" OFF)
...
# set default backend
set(AMR_MANDELBROT_KOKKOS_BACKEND "Undefined" CACHE STRING
  "Kokkos default backend device")

# Set the possible values for kokkos backend device
set_property(CACHE AMR_MANDELBROT_KOKKOS_BACKEND PROPERTY STRINGS
  "OpenMP" "Cuda" "HIP" "Undefined")

# check if user requested a build of kokkos
if(AMR_MANDELBROT_KOKKOS_BUILD)

  message("[amr_mandelbrot / kokkos] Building kokkos from source")

  # Kokkos default build options

  # set install path
  list (APPEND AMR_MANDELBROT_KOKKOS_CMAKE_ARGS
    -DCMAKE_INSTALL_PREFIX=${KOKKOS_INSTALL_DIR})

  # use predefined cmake args
  # can be override on the command line
  if (AMR_MANDELBROT_KOKKOS_BACKEND MATCHES "Cuda")
  ...
  elseif(AMR_MANDELBROT_KOKKOS_BACKEND MATCHES "HIP")
  ...
  elseif(AMR_MANDELBROT_KOKKOS_BACKEND MATCHES "OpenMP")
  ...
  elseif(AMR_MANDELBROT_KOKKOS_BACKEND MATCHES "Undefined")

    message(FATAL_ERROR "[amr_mandelbrot / kokkos] You must chose a valid AMR_MANDELBROT_KOKKOS_BACKEND !")
  ...
  if (AMR_MANDELBROT_KOKKOS_USE_GIT)
  ...
  if(TARGET Kokkos::kokkos)
    message("[amr_mandelbrot / kokkos] Kokkos found (using FetchContent)")
    set(AMR_MANDELBROT_KOKKOS_FOUND True)
    set(HAVE_KOKKOS 1)
  else()
    message("[amr_mandelbrot / kokkos] we shouldn't be here. We've just integrated kokkos build into amr_mandelbrot build !")
  endif()

  set(AMR_MANDELBROT_KOKKOS_BUILTIN TRUE)
  ...
  if(TARGET Kokkos::kokkos)
    ...
    if(KOKKOS_DEVICE_ENABLE_CUDA)
      set(AMR_MANDELBROT_KOKKOS_BACKEND "Cuda")
    ...
    elseif(KOKKOS_DEVICE_ENABLE_HIP)
      set(AMR_MANDELBROT_KOKKOS_BACKEND "HIP")
    elseif(KOKKOS_DEVICE_ENABLE_OPENMP)
      set(AMR_MANDELBROT_KOKKOS_BACKEND "OpenMP")
    endif()

    message("[amr_mandelbrot / kokkos] Kokkos found via find_package; default backend is ${AMR_MANDELBROT_KOKKOS_BACKEND}")
    set(AMR_MANDELBROT_KOKKOS_FOUND True)
    set(HAVE_KOKKOS 1)

  else()

    message(FATAL_ERROR "[amr_mandelbrot / kokkos] Kokkos is required but not found by find_package. Please adjust your env variable CMAKE_PREFIX_PATH (or Kokkos_ROOT) to where Kokkos is installed on your machine !")
