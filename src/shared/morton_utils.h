 ...
 * Seperate bits from a given integer "dim" positions apart and inserting zeros in between.
 *
...
template<int dim>
KOKKOS_INLINE_FUNCTION
uint64_t morton_key(const index_t<dim>& index)
{
  uint64_t key = 0;
  if (dim == 1) {
    key = index[IX];
...
KOKKOS_INLINE_FUNCTION
uint64_t morton_key(const int ix, const int iy)
{
  uint64_t key = 0;
