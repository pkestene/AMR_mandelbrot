...
protected:
  ...
  VTK_WRITE_ENUM m_vtk_write_type; /**< ascii, appended binary or base64 binary */
  std::string m_write_type_str; /**< a string identifying the type of vtk file format (ascii, appended or binary) */
...
public:
  ...
  bool vtk_ascii_enabled() {return m_vtk_write_type == VTK_WRITE_ENUM::ASCII; };
  bool vtk_appended_enabled() {return m_vtk_write_type == VTK_WRITE_ENUM::APPENDED; };
  bool vtk_binary_enabled() {return m_vtk_write_type == VTK_WRITE_ENUM::BINARY; };
  ...
  void write_metadata(int iStep, double time);
   ...
   */
  template<int dim>
  void write_geometry(const std::vector<Point<3> >& nodes_location = std::vector<Point<3> >());
   ...
   *
   * VERY important: we assume an implicit connectivity; more precisely, all vertex 
   * are redundant (we will do better later)
   *
   */
  template<int dim>
...
void VTKWriter::write_geometry(const std::vector<Point<3> >& nodes_location)
{
  ...
  m_out_file << "  <Points>\n";
  m_out_file << "    <DataArray type=\"Float32\" Name=\"Points\" NumberOfComponents=\"3\" format=\""
	     << m_write_type_str << "\"";
  ...
  if ( vtk_ascii_enabled() ) {
    ...
    for (uint64_t i=0; i<nbNodes; ++i) {
      ...
      m_out_file << nodes_location[i][IX] << " "
		 << nodes_location[i][IY] << " "
		 << nodes_location[i][IZ] << "\n";
...
// ================================================================
template<int dim>
void VTKWriter::write_appended_binary_geometry(std::vector<Point<3> >& nodes_location)
{
